...
void setup()
{
    ...
    rudder.attach(RUDDER_PIN, MIN_WIDTH, MAX_WIDTH);
    elevator.attach(ELEVATOR_PIN, MIN_WIDTH, MAX_WIDTH);
// #ifdef DEBUG
//     // 备用调试灯
...
void loop()
{
    ...
    for (unsigned int channel = 1; channel <= 9; ++channel) {
        ...
        case AIL:
            ...
            break;
        case ELE:
            elevator.write((int)(180 * normalized_value));
            break;
        case THR:
            {
            int thrusterValue = (int)((255 - 150) * normalized_value + 159);
            analogWrite(THRUSTER_PIN, thrusterValue);
#ifdef DEBUG
            // 测试：串口向上位机输出当前油门输出值
        ...
        case RUD:
            // 偏航代码
            rudder.write((int)(180 * normalized_value));
            break;
        case SA:
            // 按下后解锁，再按锁定
            if (value > RADIO_HIGH - DELTA_RADIO && value < RADIO_HIGH + DELTA_RADIO) { // 高位
